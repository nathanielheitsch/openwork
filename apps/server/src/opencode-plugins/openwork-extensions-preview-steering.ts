...
For Google Drive uploads and Gmail attachments, Cloud search results alone do not establish that upload is unavailable. When OpenWork host tools are exposed, first query extension.actions with extensionId openwork-cloud-uploads. Only if drive_upload_file is returned and the user authorizes the upload, execute that exact action through openwork_execute id extension.call with extensionId openwork-cloud-uploads and args containing path and optional folderId. This host action supports files up to 4 MiB under authorized roots and preserves bytes outside model context. For Gmail attachments, only if gmail_create_draft_with_attachments is returned and the user authorizes draft creation, execute that exact action through the same bridge with authorized paths and draft fields from its returned input schema. It creates a reviewable draft, does not send email, and supports up to 10 files totaling 4 MiB. The bridge uses the Cloud backend member connection, not local Google credentials. It uses the member's default Google connection and cannot select another named connection; do not substitute it for a requested account unless it is confirmed to be the default. Never load file bytes into model context or tool arguments, extract tokens, or use shell uploads as a fallback. If host tools or the action are absent, including in external MCP-only clients, report that limitation rather than claiming this host upload route is available.`;

export const OPENWORK_EXTENSION_DISCOVERY_INSTRUCTION =
  `If the user asks for something you cannot do with obvious built-in tools, check OpenWork extensions before saying the capability is unavailable. Use openwork_query with id extension.actions to inspect available extension actions, then openwork_execute with id extension.call for the matching action. ${OPENWORK_GOOGLE_CONNECTION_INSTRUCTION}`;
