...
test("search_capabilities exposes query parameter constraints as JSON schema instead of leaked validator internals", async () => {
  ...
  const draftSchema = expectRecord(draftMatch.bodySchema, "Gmail draft body schema")
  const draftProperties = expectRecord(draftSchema.properties, "Gmail draft body properties")
  expect(draftProperties.to).toMatchObject({ type: "string", minLength: 3, maxLength: 320 })
  expect(draftProperties.cc).toMatchObject({ type: "string", minLength: 3, maxLength: 1_000 })
  expect(draftProperties.bcc).toMatchObject({ type: "string", minLength: 3, maxLength: 1_000 })
  expect(draftProperties.subject).toMatchObject({ type: "string", minLength: 1, maxLength: 500 })
})
