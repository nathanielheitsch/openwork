...
const provider = { id: providerId, organization_id: orgId, created_by_org_membership_id: memberId,
  provider_id: "google-vertex", name: "Token lifecycle scratch", credential_mode: "member" as const,
  status: "active" as const, oauth_client_id: "test-client", oauth_client_secret: "test-client-secret", provider_config: {}, settings: {} }
await db.insert(MemberTable).values({ id: memberId, organizationId: orgId, userId: createDenTypeId("user") })
await db.insert(InferenceProviderTable).values(provider)
...
await db.insert(table).values({ id: credentialId, inference_provider_id: providerId, organization_id: orgId, subject: memberId, org_membership_id: memberId,
...
const scope = { credentialId, provider, subject: memberId }
...
app.post("/refresh", async (c) => {
  const result = await refresh({ credential: snapshot, token: { accessToken: "old", refreshToken: "old-refresh" }, provider, subject: memberId, now: new Date() })
  return c.json(result)
})
app.post("/resolve", async (c) => c.json(await resolveUpstreamCredential({ provider, orgMembershipId: memberId, envNames: [],
...
app.post("/change/:action", async (c) => {
  ...
  if (action === "reset") {
    mode = "success"
    await db.update(InferenceProviderTable).set({ oauth_client_secret: provider.oauth_client_secret }).where(eq(InferenceProviderTable.id, providerId))
    ...
    await db.transaction(async (tx) => {
      await tx.select().from(InferenceProviderTable).where(eq(InferenceProviderTable.id, providerId)).for("update")
      if (action === "client-rotation") await tx.update(InferenceProviderTable).set({ oauth_client_secret: "rotated-client" }).where(eq(InferenceProviderTable.id, providerId))
  ...
  else if (action === "remove-access") await db.delete(InferenceProviderAccessTable).where(eq(InferenceProviderAccessTable.inference_provider_id, providerId))
